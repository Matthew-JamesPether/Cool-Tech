...
export const getRepoData = async (req, res) => {
  try {
    // Queries which OU and division was selected
    const ou = await ous
      .find({ _id: req.query.ouId })
      .select("description -_id");
    const division = await divisions
      .find({ _id: req.query.divisionId })
      .select("description -_id");
    ...
    // Responses with the filtered data and type selected
    res.status(200).send({
      names: { ou: ou[0].description, division: division[0].description },
      data: filterData,
    });
  } catch (err) {
    res.status(400).json({ error: err.message });
